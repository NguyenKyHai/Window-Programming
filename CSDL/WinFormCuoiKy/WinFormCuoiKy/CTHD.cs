    ...
    public partial class CTHD : Form
    {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
           ...
           MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (thongbao == DialogResult.Yes)
            {
                try
                {
                    ChiTietHoaDonBLL.InsertUpdate(txtMaHD.Text, txtMaXe.Text, int.Parse(txtSoLuong.Text));
                    DataBind();
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (thongbao == DialogResult.Yes)
            {
                try
                {
                   ChiTietHoaDonBLL.Delete(txtMaHD.Text,txtMaXe.Text);
                    DataBind();
                    MessageBox.Show("Cập nhật thành công");
                }
                catch (SqlException ex)
        ...
        private void dgvCTHD_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtMaXe.Text = dgvCTHD.Rows[r].Cells[1].Value.ToString();
